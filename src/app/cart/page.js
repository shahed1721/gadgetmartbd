        ...
        {cart.length === 0 ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              {cart.map((item) => {
                // ইমেজের ডোমেইন অটোমেটিক নতুন ডোমেইনে কনভার্ট করার লজিক
                let rawImg = item.image || '/logo.png';
                if (rawImg.includes('gadgetmartbd.shop') && !rawImg.includes('admin.gadgetmartbd.shop')) {
                  rawImg = rawImg.replace('gadgetmartbd.shop', 'admin.gadgetmartbd.shop');
                }
                  ...
                  <div key={item.id} className="flex items-center justify-between border-b border-gray-100 pb-4 gap-4">
                    <div className="flex items-center gap-4">
                      <div className="relative w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0 border">
                        <Image src={rawImg} alt={item.name} fill className="object-cover" />
                      </div>
                      <div>
