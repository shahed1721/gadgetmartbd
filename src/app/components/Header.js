...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* মেইন স্লাইড মেনু */}
      <div className={`fixed top-0 left-0 h-full w-[280px] bg-white z-[101] shadow-2xl transform transition-transform duration-300 md:hidden flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        {/* মেনু হেডার */}
        <div className="bg-teal-700 flex items-center justify-between px-4 py-4 text-white">
          <span className="font-bold text-lg">Menu</span>
          <button onClick={() => setIsMobileMenuOpen(false)} className="text-3xl leading-none hover:text-teal-200 transition-colors">&times;</button>
        </div>
        
        {/* মেনু লিংকস ও ক্যাটাগরি */}
        <div className="flex-1 overflow-y-auto px-4 py-2">
          <nav className="flex flex-col gap-1 border-b border-gray-100 pb-4 mb-4 mt-2">
            <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="py-2.5 text-gray-800 font-medium hover:text-teal-600 flex items-center gap-2">🏠 Home</Link>
            <Link href="/shop" onClick={() => setIsMobileMenuOpen(false)} className="py-2.5 text-gray-800 font-medium hover:text-teal-600 flex items-center gap-2">🛍️ Shop</Link>
            <Link href="/flash-sale" onClick={() => setIsMobileMenuOpen(false)} className="py-2.5 text-yellow-500 font-bold hover:text-yellow-600 flex items-center gap-2">⚡ Flash Sale</Link>
            <Link href="/track-order" onClick={() => setIsMobileMenuOpen(false)} className="py-2.5 text-gray-800 font-medium hover:text-teal-600 flex items-center gap-2">🚚 Track Order</Link>
          </nav>

          <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">All Categories</div>
          <div className="flex flex-col gap-1 pb-6">
            {loadingCats ? (
              ...
              categories.map((cat) => (
                ...
                >
                  <span>{cat.name}</span>
                  <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">{cat.count}</span>
                </Link>
              ))
            ) : (
